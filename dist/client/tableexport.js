...
;(function (root, factory) {
...
})(undefined, function ($, Blob, saveAs, XLSX) {
    'use strict';
    /**
    ...
    var TableExport = function TableExport(selectors, options) {
        var self = this;
        ...
        self.selectors.forEach(function (el) {
            ...
            context.filename = settings.filename === 'id' ? el.getAttribute('id') ? el.getAttribute('id') : self.defaultFilename : settings.filename ? settings.filename : self.defaultFilename;
    ...
    TableExport.prototype = {
        ...
        exporters: {
            build: function build(context, key) {
                var self = this;
                var settings = self.settings;
                ...
                var getReturn = function getReturn(val) {
                    if (_isEnhanced(key)) {
                        return {
         ...
         * @returns Element
         */
        createObjButton: function createObjButton(hashKey, dataObject, myContent, myClass, bootstrapSettings) {
         ...
         */
        formatValue: function formatValue(isTrimWhitespace, string) {
         ...
         */
        getType: function getType(string) {
            if (!string) return '';
         ...
         * @param merges {Object[]}
         */
        createSheet: function createSheet(data, merges) {
            var ws = {};
         ...
         * @memberof TableExport.prototype
         */
        downloadHandler: function downloadHandler(event) {
         ...
         * @returns {ArrayBuffer}
         */
        string2ArrayBuffer: function string2ArrayBuffer(s) {
         ...
         * @param merges {Object[]}
         */
        export2file: function export2file(data, mime, name, extension, merges) {
        ...
        downloadDataURI: function downloadDataURI(dataURI, name, extension) {
        ...
        getBookType: function getBookType(key) {
            switch (key) {
                case _FORMAT.XLS:
        ...
        getRawData: function getRawData(data, extension, name, merges) {
        ...
        getFileSize: function getFileSize(data, extension) {
     ...
     * @constructor
     */
    var RowColMap = function RowColMap() {
        this.rcMap = [];
        this.merges = [];
    ...
    RowColMap.prototype = {
        ...
        build: function build(context, settings) {
            var self = this;
            ...
            var handleIgnore = function handleIgnore(ir, ic) {
                self.setRowColMapProp(ir, ic, self.TYPE.IGNORE, true);
            };
            var handleEmpty = function handleEmpty(ir, ic) {
                self.setRowColMapProp(ir, ic, self.TYPE.EMPTY, true);
            };
            var handleRowSpan = function handleRowSpan(val, ir, ic) {
            ...
            var handleColSpan = function handleColSpan(val, ir, ic, isRowSpan, rowSpan) {
            ...
            var handleMerge = function handleMerge(irs, ics, ire, ice) {
                var merge = {
                    s: { r: irs, c: ics },
                    e: { r: ire, c: ice }
                };
    ...
    var _isEnhanced = function () {
        return function (key) {
            return XLSX && !_isMobile && !TableExport.prototype.formatConfig[key].enforceStrictRFC4180;
        };
    }();
